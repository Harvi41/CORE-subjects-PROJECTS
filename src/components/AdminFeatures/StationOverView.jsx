...
function getStatus(r) {
  const compareVal = r.isTemp ? r.value : (r.value / r.max) * 100;
  const warnPct    = r.isTemp ? r.thresholdWarn  : (r.thresholdWarn  / r.max) * 100;
  const dangerPct  = r.isTemp ? r.thresholdDanger: (r.thresholdDanger/ r.max) * 100;
  if (compareVal <= dangerPct) return "danger";
  if (compareVal <= warnPct)   return "warning";
  return "normal";
}
function getBarPct(r) { return Math.min(100, (r.value / r.max) * 100); }
...
export default function StationOverview() {
  ...
  const handleSave = () => {
    ...
    if (isNaN(num) || num < 0) { setEditErr("Enter a valid positive number."); return; }
    if (num > editTarget.max)  { setEditErr(`Max allowed is ${editTarget.max} ${editTarget.unit}.`); return; }
    setResources((prev) => prev.map((r) => r.id === editTarget.id ? { ...r, value: num, lastUpdated: "Just now" } : r));
  ...
  const quickAdjust = (id, delta) => {
    setResources((prev) =>
      prev.map((r) => {
        if (r.id !== id) return r;
        const next = Math.min(r.max, Math.max(0, parseFloat((r.value + delta).toFixed(2))));
        return { ...r, value: next, lastUpdated: "Just now" };
      })
    );
  };
  ...
  const handleAddSave = () => {
    ...
    const newItem = {
      ...
      thresholdDanger: +addForm.thresholdDanger,
    };
    ...
    <div className="so-root">
      ...
      <div className="so-grid">
        {filtered.map((r, i) => {
          ...
          const step   = r.isTemp ? 0.1 : r.unit === "%" ? 1 : r.unit === "°C" ? 0.5 : Math.ceil(r.max / 100);

          return (
            <div key={r.id} className={`so-card so-card-${status}`} style={{ animationDelay: `${i * 0.05}s` }}>
