...
export default function MedicalInventory() {
  ...
  const handleAdd = () => {
    ...
    const id = `MED-${String(itemCounter++).padStart(3, "0")}`;
    setItems((prev) => [{ id, ...form, quantity: +form.quantity }, ...prev]);
    closeModal();
  ...
  const handleUpdate = () => {
    const e = validate();
    if (Object.keys(e).length) { setErrors(e); return; }
    setItems((prev) => prev.map((i) => i.id === selected.id ? { ...i, ...form, quantity: +form.quantity } : i));
    closeModal();
  ...
  const handleQtySave = (item) => {
    const val = +qtyEdit[item.id];
    if (isNaN(val) || val < 0) return;
    setItems((prev) => prev.map((i) => i.id === item.id ? { ...i, quantity: val } : i));
    setQtyEdit((p) => { const n = { ...p }; delete n[item.id]; return n; });
    showToast(`Quantity updated for "${item.name}".`);
  };

  // remove all expired
  const removeAllExpired = () => {
    const count = items.filter((i) => getExpiryStatus(i.expiryDate) === "expired").length;
    setItems((prev) => prev.filter((i) => getExpiryStatus(i.expiryDate) !== "expired"));
    showToast(`${count} expired item${count !== 1 ? "s" : ""} removed.`, "warning");
  };
    ...
    <div className="mi-root">
      ...
      {expiringCount > 0 && (
        <div className="mi-alert mi-alert-warning">
          ⚠️ <strong>{expiringCount} item{expiringCount !== 1 ? "s are" : " is"} expiring within 30 days.</strong> Schedule restocking.
        </div>
      )}
